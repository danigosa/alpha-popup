    ...
    var FooterPopup = {
        ...
        showMessage: function() {
            ...
            if ( this.options.loadStyles !== "false" )
                this.$styling = $('<style type="text/css">#pusher {height: 50px;width:100%} .FooterPopup { z-index: 10000 !important;font-size: 15px; position: fixed; left: 10px; border-radius: 5px; -moz-border-radius: 5px; -webkit-border-radius: 5px; -o-border-radius: 5px; background-color: '+ this.options.bgcolor +'; color: '+ this.options.color +'; } .FooterPopuphead { top: 0; } .FooterPopupfooter { bottom: 10px; } .FooterPopup .container {text-align: left;padding: 10px} .FooterPopup p {position: relative; padding: 0; margin: 0 0 0 40px } .AlphaPopup a {color: '+ this.options.color +' ; text-decoration: underline; } .AlphaPopupClose { background: url("https://s3.amazonaws.com/wsscripts/close.png") no-repeat top center; background-size: 30px;opacity:0.4;float: left; width: 30px;height: 40px } .AlphaPopupClose:hover {opacity: 1} @media screen and (max-width: 1000px) { .AlphaPopup .container {width: 100%} }</style>').appendTo('head');

            /** Append message to body */
            this.$message = $('<div id="FooterPopup" class="FooterPopup FooterPopup'+ this.options.position +'" style="display: none;"><div class="container"><a href="#" class="AlphaPopupClose"></a><p>'+ this.options.message +' </p></div></div>').appendTo('body').slideDown({ duration: 400, easing: 'swing' });
